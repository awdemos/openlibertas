...
pub fn command_description(cmd: &str) -> &'static str {
    match cmd {
        ...
        "/copy" => "Copy a message to clipboard via OSC 52",
...
pub fn command_detailed_help(cmd: &str) -> Option<String> {
    ...
    let usage = match cmd {
        ...
        "/copy" => "Usage: /copy [n]\nCopy the nth message (or the last assistant message if no index is given) to the terminal clipboard via OSC 52.",
