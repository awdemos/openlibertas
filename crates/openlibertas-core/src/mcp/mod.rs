...
pub use transport::{McpServerType, McpTransport, McpTransportError};
...
impl std::fmt::Debug for McpClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut dbg = f.debug_struct("McpClient");
        dbg.field("servers", &self.servers);
        if let Ok(transports) = self.transports.try_lock() {
            dbg.field("transports", &transports.keys().collect::<Vec<_>>());
        } else {
            dbg.field("transports", &"<locked>");
        }
        if let Ok(statuses) = self.server_statuses.try_lock() {
            dbg.field("server_statuses", &*statuses);
        } else {
            dbg.field("server_statuses", &"<locked>");
        }
        if let Ok(errors) = self.server_errors.try_lock() {
            dbg.field("server_errors", &*errors);
        } else {
            dbg.field("server_errors", &"<locked>");
        }
        dbg.finish()
    }
}
...
impl McpClient {
    ...
    pub async fn discover_tools(&self) -> Result<Vec<McpTool>> {
        ...
        for (name, config) in &self.servers {
            ...
            let transport: Box<dyn McpTransport> = match config.server_type.as_str() {
                "local" => {
                    let cmd = config
                        .command
                        .as_ref()
                        .context("Local MCP server missing command")?;
                    Box::new(LocalTransport::new(cmd.clone()))
                }
                "remote" => {
                    let url = config
                        .url
            ...
            match transport.discover_tools().await {
                Ok(tools) => {
